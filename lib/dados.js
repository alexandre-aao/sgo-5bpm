...
const TABELAS = ['usuarios', 'sessoes', 'bairros_coordenadas', 'pessoal', 'eventos', 'operacoes', 'alocacoes', 'escalas', 'cartoes', 'viaturas', 'cartao_grupos_modelo_versoes'];
...
/** Compatibilidade para consumidores antigos. A escolha do Cartão Ordinário não
 *  depende mais do dia da semana. */
...
async function buscarPadroesAtivos() {
  const { data, error } = await supabase
    ...
    .eq('tipo_modelo', 'ordinario');
  if (error) throw new Error(`Falha ao buscar cartão padrão: ${error.message}`);
  return data || [];
}

/** O Modelo Ordinário publicado que deve originar qualquer cartão do dia. */
async function buscarPadraoAtivo(_dataCartao = null) {
  const padroes = await buscarPadroesAtivos();
  if (padroes.length === 0) return null;
  // Resolve o padrão ativo para a fotografia publicada. Assim, edições no
  // rascunho nunca alteram cartões do dia antes de uma nova publicação.
