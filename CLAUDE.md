...
- **Drawers/off-canvas:** dois componentes distintos, não confundir — `#drawer` (`.drawer`/`.drawer-overlay`/`.drawer-content`) é a gaveta de detalhes de evento, aberta por `openDrawer(eventId)`/`closeDrawer()`; `.sidebar`/`#nav-drawer-overlay` (classes `.nav-drawer-open`/`.open`, `abrirNavDrawer()`/`fecharNavDrawer()`) é o menu de navegação mobile, só ativo em `@media (max-width:768px)` — em desktop a sidebar continua fixa, sem drawer. Ambos usam a mesma técnica visual (wrapper fixo + overlay com fade + painel deslizando via `transition`), mas com nomes de classe/função próprios de cada um.
