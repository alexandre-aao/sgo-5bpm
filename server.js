...
app.get('/api/arquivamento/previa', exigirP3, asyncRoute(async (req, res) => {
...
app.post('/api/arquivamento/exportar', exigirP3, asyncRoute(async (req, res) => {
...
app.post('/api/arquivamento/executar', exigirP3, asyncRoute(async (req, res) => {
  const ate = String(req.body.ate || '');
  const erro = validarDataArquivamento(ate);
  if (erro) return res.status(400).json({ error: erro });
