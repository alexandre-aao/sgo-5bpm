...
import { dataBr, dataHoraBr, rotuloPeriodo } from './utils';
      ...
      <aside className="drawer-content" role="dialog" aria-modal="true" aria-label="Detalhes da alteração">
        ...
        <div className="drawer-body">
          <div className="alteracoes-detalhe-topo">
            <span className="badge badge-info">{alteracao.jornada === '12H' ? 'Jornada 12h' : 'Jornada 24h'}</span>
          ...
          <section className="drawer-section"><h3><AlertTriangle /> Plantões afetados</h3>{alteracao.impacto ? <><div className="alteracoes-impacto-metricas"><div className="panel-subsection"><strong>{alteracao.impacto.dias_corridos ?? '—'}</strong><span className="texto-auxiliar">dias corridos</span></div><div className="panel-subsection"><strong>{alteracao.impacto.servicos_atingidos ?? 0}</strong><span className="texto-auxiliar">plantões atingidos</span></div></div><dl className="drawer-dl alteracoes-impacto-dl"><dt>Serviços atingidos</dt><dd>{alteracao.impacto.datas_servicos?.length ? alteracao.impacto.datas_servicos.map(dataBr).join(' · ') : 'Nenhum plantão projetado'}</dd><dt>Próximo serviço</dt><dd>{dataBr(alteracao.impacto.proximo_servico)}</dd>{alteracao.jornada === '12H' && <><dt>Intervalo do turno</dt><dd>{alteracao.horario_inicio || '—'}–{alteracao.horario_fim || '—'} · {alteracao.turno}</dd></>}</dl></> : <p className="panel-nota">O cálculo de impacto estará disponível após a composição da Companhia ser informada.</p>}</section>

          <section className="drawer-section"><h3><ShieldCheck /> Ciência do Adjunto</h3>{ciente ? <div className="panel-nota"><CheckCircle2 /> Ciente por <strong>{alteracao.ciencia?.nome || alteracao.ciencia?.usuario_nome || alteracao.ciencia?.usuario || alteracao.ciente_por}</strong> em {dataHoraBr(alteracao.ciencia?.data_hora || alteracao.ciencia?.criado_em || alteracao.ciente_em)}. Esta ciência não encerra nem altera o registro.</div> : <div className="acoes-linha"><span className="texto-auxiliar">Pendente — o Adjunto ainda não tomou ciência.</span>{podeCiencia && <button type="button" className="btn btn-primary btn-sm" onClick={() => void marcarCiente()} disabled={enviandoCiencia}><CheckCircle2 /> Registrar ciência</button>}</div>}</section>
